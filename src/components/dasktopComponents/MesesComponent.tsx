...
const MesesComponent =  ({ onMesSelecionado }: any) => {
  ...
  return(
    <span>
        <div className={`flex space-x-3 px-4 overflow-x-auto py-2 ${mode ==='true' ? 'bg-dark' : 'bg-azulEscuro'} `} >
          {meses.map((mes) => (
            <button
                key={mes.valor}
                className={`px-4 min-w-24 h-7 ${mode === 'true' ? 'bg-darkClaro' : 'bg-azulEscuro'} rounded-full p-1 items-center font-semibold fonte-inter text-16 text-white justify-center ${
